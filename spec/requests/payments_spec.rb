...
describe "Payment Request Processing", type: :request do
  let!(:property) { Property.create(address1: "123 Test St", bedrooms: 4, bathrooms: 2.5, zipcode: 12345) }
  let!(:user) { User.create(email: "123@test.com", password: "abcd1234", password_confirmation: "abcd1234", confirmed_at: DateTime.now) }
  let!(:headers) { user.create_new_auth_token.merge({ "CONTENT_TYPE" => "application/json", "ACCEPT" => "application/json" }) }
  ...
  it "creates a payment request with all pertinate info" do
    ...
    property_request = PaymentRequest.find(json_body["id"])
